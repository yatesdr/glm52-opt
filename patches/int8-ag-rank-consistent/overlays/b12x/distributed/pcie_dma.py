...
def _fp8_mode() -> str:
    ...
    "i8": keep the bf16 reduce-scatter and use symmetric signed INT8 only
    for the allgather payload. The wire layout remains one payload byte per
    value plus one fp32 scale per 128 values.

    Every compressed mode materializes the locally owned reduced shard
    through the same wire payload as its peers. An all-reduce result must be
    rank-identical; retaining a pre-wire BF16 owner shard while peers
    dequantize that shard gives every TP rank a different replicated
    activation.
    """
...
def _normalize_fp8_mode(value: str | None) -> str:
    ...
    if raw in ("a2a", "ring"):
        return raw
    if raw in ("i8", "int8", "ag_i8", "int8_ag"):
        return "i8"
    return "ag"
...
class PCIeDmaAllReduce:
    ...
    def __init__(
        ...
        *,
        ...
        self.wire_mode = (
            "int8-ag"
            if self._fp8 == "i8"
            else (f"fp8-{self._fp8}" if self._fp8 else "bf16")
        )
    ...
    def all_reduce(
        ...
        if compressed_eligible and self._fp8 == "a2a":
            return self._all_reduce_fp8(inp, out, shard_elems)
        fp8_ring = compressed_eligible and self._fp8 == "ring"
        compressed_ag = compressed_eligible and self._fp8 in (
            "ag",
            "ring",
            "i8",
        )
        int8_ag = compressed_eligible and self._fp8 == "i8"
        quantize_ag = ext.dma_quant_i8 if int8_ag else ext.dma_quant
        dequantize_ag = (
            ext.dma_dequant_store_i8 if int8_ag else ext.dma_dequant_store
        )
        ...
        # Compressed AG slices are piece-contiguous: [payload][scales] per piece.
        ...
        for k in range(steps):
            ...
            fp8_step = fp8_reduce or (compressed_ag and not reduce_phase)
