...
class PCIeDmaAllReduce:
    ...
    def reduce_scatter(
        ...
        """Reduce-scatter phase only (no all-gather).  world-1 ring hops.

        When fp8="ring": first hop quantizes the caller's input, each hop
        runs dma_dequant_add_quant, the final hop materializes BF16 via
        StoreBf16=True.  When fp8="": bf16 wire with dma_add accumulation.
        """
        if not self.should_allreduce(inp):
            raise ValueError(
                "input does not satisfy ring RS requirements "
                f"(shape={tuple(inp.shape)}, dtype={inp.dtype})"
            )
        if out is None:
            # In-place RS supported and preferred: accumulation reads inp
            # chunks only for their k==0 send slot; element-wise adds alias
            # safely. Callers wanting a separate buffer still pass out.
            out = inp
        ext = self._ext
        world = self.world_size
        rank = self.rank
        ...
        fp8_wire = fp8_eligible and self._fp8 in ("ring", "ag")
        base = out.data_ptr()

        pieces = self._pick_pieces(shard_elems, shard_bytes)
        if fp8_wire and (shard_elems // pieces) % FP8_QUANT_BLOCK != 0:
            pieces = 1
        piece_elems = shard_elems // pieces
        piece_bytes = piece_elems * elem
        piece_slice_bytes = piece_elems + piece_elems // FP8_QUANT_BLOCK * 4
        ...
        def piece_ptr(chunk, piece):
            return base + chunk * shard_bytes + piece * piece_bytes
        def in_piece_ptr(chunk, piece):
            return in_base + chunk * shard_bytes + piece * piece_bytes
        def scratch_piece(owner, step, piece):
            return self._scratch_ptr(owner, step) + piece * piece_bytes
        def fp8_scratch_piece(owner, step, piece):
            return self._scratch_ptr(owner, step) + piece * piece_slice_bytes
        def slot(step, piece):
            return step * pieces + piece
        ...
        if fp8_wire:
            ...
            def fp8_stage_piece(chunk, piece):
                return stage + chunk * stride + piece * piece_slice_bytes

        for k in range(rs_steps):
            # shift=-1 vs allreduce RS phase: leaves rank r holding
            # fully-reduced chunk r (CPU-simulated, all 4 ranks verified)
            send_chunk = (rank - k - 1) % world
            recv_chunk = (rank - k - 2) % world
            fp8_step = fp8_wire
            for p in range(pieces):
                if fp8_step:
                    send_src = fp8_stage_piece(send_chunk, p)
                ...
                if fp8_step:
                    ...
                    ext.dma_dequant_add_quant(
                        ...
                        reduced + piece_elems,
                        piece_elems,
                        k == rs_steps - 1,  # StoreBf16 on final hop
                    )
                else:
                    ext.dma_add(
                        piece_ptr(recv_chunk, p),
        ...
        main.wait_stream(copy_stream)
        main.wait_stream(flag_stream)
        # Neighbor handshake: protect scratch from reuse while peers read
        done = rs_steps * pieces
        ext.dma_set_flag(
            self._flag_ptr((rank - 1) % world, done),
            self._counter_ptr(self._send_counters, done),
        )
        ext.dma_wait_flag(
            self._flag_ptr(rank, done),
