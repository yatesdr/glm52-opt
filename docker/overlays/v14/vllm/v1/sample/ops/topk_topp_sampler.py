...
def apply_top_k_top_p(
    ...
    # The PyTorch sort fallback materializes multiple batch-by-vocabulary
    # tensors.  Even at decode-sized batches that transient can exhaust the
    # deliberately tight serving headroom, while the Triton implementation
    # uses its bounded workspace for every supported batch size.
    if HAS_TRITON:
        return apply_top_k_top_p_triton(logits, k, p)
