...
class AutoRegressiveSpeculator(DraftModelSpeculator):
    ...
    def propose(
        ...
        with record_function_or_nullcontext("vllm:v2/speculator/prepare"):
            ...
            # that blocks on cudaStreamSynchronize every draft step (~16 ms
            # of CPU run-ahead, ~4.6 ms true GPU idle per step). Each draft
            # step advances every sequence by exactly one token, so the
            # target-side upper bound plus num_speculative_tokens stays a
            # valid (>=) bound for all steps; the indexer marks invalid tail
            # entries -1 and MLA clamps to topk, so an optimistic bound is
            # safe (same invariant the draft-prefill path relies on).
