...
def get_dcp_local_seq_lens(
    ...
    if dcp_rank is None:
        ...
        # Cache the constant [[dcp_rank]] tensor. Building it fresh with
        # torch.tensor(..., device=cuda) issues a pageable H2D copy that
        # synchronizes the CPU with the stream every call; inside the MTP
        # draft loop that stall (~10 ms/step) destroys async run-ahead.
