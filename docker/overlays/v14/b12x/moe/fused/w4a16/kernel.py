    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            raise ValueError("W4A16 hybrid direct tier children disagree on tile counts")
        if (
            self.nv_fc1.top_k != 8
    ...
    def _run_persistent_gemm_hybrid_direct(
        ...
        while work_mn_tile < global_mn_tiles:
            ...
            if nv_local_expert >= Int32(0):
                ...
                if (
                    nf_local_expert >= Int32(0)
                    and nf_local_expert < Int32(self.nf_num_experts)
                ):
                    if cutlass.const_expr(is_fc1):
...
class W4A16HybridMappedKernel(W4A16HybridDirectKernel):
    """Exact one-grid hybrid kernel with device-side global-expert mapping."""
...
_HYBRID_DIRECT_CACHE: dict[tuple, W4A16HybridDirectCompileResult] = {}
...
_HYBRID_MAPPED_CACHE: dict[tuple, W4A16HybridMappedCompileResult] = {}
...
def _hybrid_direct_query_kernel_resources(
    ...
    if registers_per_thread < 0 or local_memory_bytes < 0:
        raise RuntimeError(
            "W4A16 hybrid direct CUDA function attributes are incomplete for "
            f"{symbol}"
        )
...
def compile_w4a16_hybrid_direct(
    *,
    size_m: int,
    hidden_size: int,
    intermediate_size: int,
    ...
    max_shared_mem: int,
    force_tile_config: tuple[int, int, int, int],
) -> W4A16HybridDirectCompileResult:
    """Compile the exact one-grid production heterogeneous direct kernel."""

    if not torch.cuda.is_available():
        raise RuntimeError("W4A16 hybrid direct compilation requires CUDA")
    cutlass_dtype = _cutlass_element_dtype(element_dtype)
    device = int(torch.cuda.current_device())
    kernel = W4A16HybridDirectKernel(
        size_m=size_m,
        hidden_size=hidden_size,
        intermediate_size=intermediate_size,
    ...
    if kernel.shared_words != 11_296:
        raise ValueError(
            "W4A16 hybrid direct v1 requires exactly 45,184 bytes of shared "
            f"memory, got {kernel.shared_words * 4}"
        )
    ...
    cache_key = (
        ...
        device,
        int(max_shared_mem),
        "direct_cute_compile_keep_cubin",
        kernel.__cache_key__,
    )
    cached = _HYBRID_DIRECT_CACHE.get(cache_key)
    if cached is not None:
        return cached

    compile_spec = KernelCompileSpec.from_key(
        "moe.w4a16.hybrid_direct",
        2,
        cache_key,
    )
    a_fake = make_ptr(cutlass_dtype, 16, cute.AddressSpace.gmem, assumed_align=16)
    ...
    nv_local_ids_fake = cute.runtime.make_fake_compact_tensor(
        cutlass.Int32,
        (W4A16HybridDirectKernel.ROUTE_SLOTS,),
        assumed_align=16,
    )
    nf_local_ids_fake = cute.runtime.make_fake_compact_tensor(
        cutlass.Int32,
        (W4A16HybridDirectKernel.ROUTE_SLOTS,),
        assumed_align=16,
    )
    ...
    if not callable(to_executor):
        raise RuntimeError(
            "W4A16 hybrid direct compile result cannot bind a CuTe executor"
        )
    ...
    result = W4A16HybridDirectCompileResult(
        compiled=compiled,
        executor=executor,
        size_m=4,
        hidden_size=6144,
    ...
    _HYBRID_DIRECT_CACHE[cache_key] = result
    return result


def compile_w4a16_hybrid_mapped(
    *,
    size_m: int,
    hidden_size: int,
    intermediate_size: int,
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("W4A16 hybrid mapped compilation requires CUDA")
    cutlass_dtype = _cutlass_element_dtype(element_dtype)
    device = int(torch.cuda.current_device())
    kernel = W4A16HybridMappedKernel(
        size_m=size_m,
        hidden_size=hidden_size,
        intermediate_size=intermediate_size,
        nv_num_experts=nv_num_experts,
    ...
    if fixed_resources != (128, 256, 1, 11_296, 754, 2_097_152, 32, 256):
        raise ValueError(
            "W4A16 hybrid mapped v1 fixed resources changed: "
            f"{fixed_resources!r}"
        )
    ...
    cache_key = (
        ...
        device,
        int(max_shared_mem),
        "mapped_cute_compile_keep_cubin",
        kernel.__cache_key__,
    )
    cached = _HYBRID_MAPPED_CACHE.get(cache_key)
    if cached is not None:
        return cached

    compile_spec = KernelCompileSpec.from_key(
        "moe.w4a16.hybrid_mapped",
        W4A16HybridMappedKernel.ABI_VERSION,
        cache_key,
    )
    a_fake = make_ptr(cutlass_dtype, 16, cute.AddressSpace.gmem, assumed_align=16)
    nv_w13_fake = cute.runtime.make_fake_compact_tensor(
    ...
    global_topk_ids_fake = cute.runtime.make_fake_compact_tensor(
        cutlass.Int32,
        (W4A16HybridMappedKernel.ROUTE_SLOTS,),
        assumed_align=16,
    )
    tier_local_map_fake = cute.runtime.make_fake_compact_tensor(
        cutlass.Int32,
        (W4A16HybridMappedKernel.MAP_SLOTS,),
        assumed_align=16,
    )
    fc1_fake = cute.runtime.make_fake_compact_tensor(
        cutlass_dtype, (32 * 1024,), assumed_align=16
    ...
    if not callable(to_executor):
        raise RuntimeError(
            "W4A16 hybrid mapped compile result cannot bind a CuTe executor"
        )
    executor = to_executor(device)
    kernel_resources = _hybrid_direct_query_kernel_resources(
        compiled,
    ...
    _HYBRID_MAPPED_CACHE[cache_key] = result
    return result
...
def clear_w4a16_kernel_cache() -> None:
    _CACHE.clear()
    _FUSED_CACHE.clear()
    _HYBRID_DIRECT_CACHE.clear()
    _HYBRID_MAPPED_CACHE.clear()
...
def _w4a16_hybrid_direct_launch_flat(
    a_input: torch.Tensor,
    ...
    nv_local_ids_i32: torch.Tensor,
    nf_local_ids_i32: torch.Tensor,
    ...
    if not a_input.is_cuda:
        raise ValueError("W4A16 hybrid direct input must be a CUDA tensor")
    device_index = a_input.device.index
    if device_index is None:
        raise ValueError("W4A16 hybrid direct input lacks a CUDA device index")
    current_device = int(torch.cuda.current_device())
    if int(device_index) != current_device:
        raise ValueError(
            "W4A16 hybrid direct input must be on the current CUDA device: "
            f"cuda:{int(device_index)} != cuda:{current_device}"
        )
    current_stream_int = int(
        torch.cuda.current_stream(a_input.device).cuda_stream
    )
    if int(stream_int) != current_stream_int:
        raise ValueError(
            "W4A16 hybrid direct stream must be the current input-device stream: "
            f"{int(stream_int)} != {current_stream_int}"
        )
    if int(m) != 4 or int(size_m) != 4:
        raise ValueError("W4A16 hybrid direct v1 requires m=size_m=4")
    if int(grid_x) != W4A16HybridDirectKernel.GRID_X:
        raise ValueError("W4A16 hybrid direct v1 requires grid_x=128")
    force_tile_config = (
        int(fc1_tile_k),
        int(fc1_tile_n),
        int(fc2_tile_k),
    ...
    hybrid = compile_w4a16_hybrid_direct(
        size_m=size_m,
        hidden_size=hidden_size,
        intermediate_size=intermediate_size,
        nv_num_experts=nv_num_experts,
    ...
    _hybrid_direct_validate_aliases(
        tuple(
            (name, tensor, mutable)
...
@torch.library.custom_op(
    "b12x::w4a16_hybrid_direct_launch",
    mutates_args="unknown",
    device_types="cuda",
)
def _w4a16_hybrid_direct_launch_op(
    a_input: torch.Tensor,
    nv_w13_i32: torch.Tensor,
    nv_w2_i32: torch.Tensor,
    nv_w13_scale_i32: torch.Tensor,
    ...
    _w4a16_hybrid_direct_launch_flat(
        a_input,
        nv_w13_i32,
        nv_w2_i32,
        nv_w13_scale_i32,
...
@_w4a16_hybrid_direct_launch_op.register_fake
def _w4a16_hybrid_direct_launch_fake(
    a_input: torch.Tensor,
    nv_w13_i32: torch.Tensor,
    nv_w2_i32: torch.Tensor,
    nv_w13_scale_i32: torch.Tensor,
...
def _w4a16_hybrid_mapped_launch_flat(
    ...
    if not a_input.is_cuda:
        raise ValueError("W4A16 hybrid mapped input must be a CUDA tensor")
    device_index = a_input.device.index
    if device_index is None:
        raise ValueError("W4A16 hybrid mapped input lacks a CUDA device index")
    current_device = int(torch.cuda.current_device())
    if int(device_index) != current_device:
        raise ValueError(
            "W4A16 hybrid mapped input must be on the current CUDA device: "
            f"cuda:{int(device_index)} != cuda:{current_device}"
        )
    current_stream_int = int(
        torch.cuda.current_stream(a_input.device).cuda_stream
    ...
    if int(stream_int) != current_stream_int:
        raise ValueError(
            "W4A16 hybrid mapped stream must be the current input-device stream: "
            f"{int(stream_int)} != {current_stream_int}"
        )
    if int(m) != 4 or int(size_m) != 4:
        raise ValueError("W4A16 hybrid mapped v1 requires m=size_m=4")
    if int(grid_x) != W4A16HybridMappedKernel.GRID_X:
        raise ValueError("W4A16 hybrid mapped v1 requires grid_x=128")
    force_tile_config = (
        int(fc1_tile_k),
        int(fc1_tile_n),
        int(fc2_tile_k),
    ...
    hybrid = compile_w4a16_hybrid_mapped(
        size_m=size_m,
        hidden_size=hidden_size,
        intermediate_size=intermediate_size,
        nv_num_experts=nv_num_experts,
...
@torch.library.custom_op(
    "b12x::w4a16_hybrid_mapped_launch",
    mutates_args="unknown",
    device_types="cuda",
)
def _w4a16_hybrid_mapped_launch_op(
    a_input: torch.Tensor,
    nv_w13_i32: torch.Tensor,
    nv_w2_i32: torch.Tensor,
    nv_w13_scale_i32: torch.Tensor,
...
@_w4a16_hybrid_mapped_launch_op.register_fake
def _w4a16_hybrid_mapped_launch_fake(
    a_input: torch.Tensor,
    nv_w13_i32: torch.Tensor,
    nv_w2_i32: torch.Tensor,
    nv_w13_scale_i32: torch.Tensor,
...
__all__ = [
    ...
    "W4A16TopKSumCompileResult",
    "W4A16FusedMoeKernel",
    "W4A16HybridDirectKernel",
    "W4A16HybridMappedKernel",
...
# Exact 188-CTA mapped kernel for the production 188-SM SM120 target. Keep a
# separate compile cache so its launch geometry cannot alias direct128 entries.
...
def w4a16_hybrid_mapped_grid188_mapping_proof() -> dict[str, object]:
    ...
    if (
        ...
        or set(fc1_flat)
        != set(range(_W4A16_HYBRID_MAPPED_GRID188_FC1_TASKS))
    ):
    ...
    if (
        ...
        or set(fc2_flat)
        != set(range(_W4A16_HYBRID_MAPPED_GRID188_FC2_TASKS))
    ):
...
def _hybrid_mapped_grid188_validate_current_device(
    *,
    ...
    if detected_sms != _W4A16_HYBRID_MAPPED_GRID188_TARGET_SMS:
        raise RuntimeError(
            "W4A16 hybrid mapped grid188 requires exactly 188 SMs, got "
            f"{detected_sms}"
        )
    ...
    device_max_shared_mem_value = getattr(
...
class W4A16HybridMappedGrid188Kernel(W4A16HybridDirectKernel):
    ...
    def __init__(
        ...
        *,
        ...
        if self.GRID_X > self.sms * self.blocks_per_sm:
            raise RuntimeError(
...
def _w4a16_hybrid_mapped_grid188_launch_flat(
    ...
    if (
        ...
        or resource_metadata.get("grid_x")
        != _W4A16_HYBRID_MAPPED_GRID188_GRID_X
        ...
        or resource_metadata.get("task_schedule")
        != "task=cta+wave*188"
        or resource_metadata.get("fc1_schedule") != "128x1+60x0"
        or resource_metadata.get("fc2_schedule") != "16x5+172x4"
    ):
        raise RuntimeError(
            "W4A16 hybrid mapped grid188 lacks exact resource admission"
        )
